...
use Inc\callbacks\CptCallBacks;

class CustomPostTypeController extends BaseController
{
    private $subPages = [];
    public $custom_post_type = [];
    protected $callbackClass = CptCallBacks::class;

    public function register()
    {
        $activated = Manager::isActive(Manager::CPT_MANAGER);
        if (!$activated) return;
    ...
    {
        $options = get_option(self::generateName('plugin_cpt'));
    ...
    private function setSubPages()
    {
        try {
            $this->subPages[] = [
                ...
                "menu_slug" => $this->generateName('cpt'),
    ...
    {
        $args = [[
            'option_group' => $this->generateName('plugin_cpt_settings'),
            'option_name' => $this->generateName('plugin_cpt'),
            'callback' => [$this->getCallBack(), 'sanitize']
        ]];
    ...
    private function setFieldsSections()
    {
        $args = [
            [
                'id' =>$this->generateName('plugin_cpt_index'),
                'title' => 'Custom Post Type Manager',
                'callback' => [$this->getCallBack(), 'section'],
                'page' => $this->generateName('cpt')
            ]
        ];
    ...
    private function setFieldsField()
    {
        $option_name =  $this->generateName('plugin_cpt');
        $page = $this->generateName('cpt');
        $default_section =  $this->generateName('plugin_cpt_index');
        $args = [
            [
                'id' => 'post_type',
                'title' => 'Custom Post Type ID',
                'callback' => array( $this->getCallBack(), 'textField' ),
                'page' => $page,
            ...
            [
                'id' => 'public',
                'title' => 'Public',
                'callback' => array( $this->getCallBack(), 'checkboxField' ),
                'page' => $page,
                'section' => $default_section,
                'args' => [
                    'option_name' => $option_name,
